...
int tcp_v4_connect_TUv1(struct sock *sk, struct sockaddr *uaddr, int addr_len, unsigned short userport)
{
	struct sockaddr_in *usin = (struct sockaddr_in *)uaddr;
	struct inet_timewait_death_row *tcp_death_row;
	struct inet_sock *inet = inet_sk(sk);
	 ...
	 */
	tcp_set_state(sk, TCP_SYN_SENT);
	err = inet_hash_connect_TUv1(tcp_death_row, sk, userport);
	if (err)
		goto failure;
...
EXPORT_SYMBOL(tcp_v4_connect_TUv1);
...
struct proto tcp_prot = {
	...
	.connect_TUv1 = tcp_v4_connect_TUv1,
