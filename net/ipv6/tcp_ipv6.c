...
static int tcp_v6_connect_TUv1(struct sock *sk, struct sockaddr *uaddr,
			  int addr_len, unsigned short userport)
{
	...
	if (addr_type & IPV6_ADDR_MAPPED) {
		...
		err = tcp_v4_connect_TUv1(sk, (struct sockaddr *)&sin, sizeof(sin), userport);
	...
	tcp_set_state(sk, TCP_SYN_SENT);
	err = inet6_hash_connect_TUv1(tcp_death_row, sk, userport);
	if (err)
		goto late_failure;
...
struct proto tcpv6_prot = {
	...
	.connect_TUv1 = tcp_v6_connect_TUv1,
