...
static int dccp_v6_connect_TUv1(struct sock *sk, struct sockaddr *uaddr,
			   int addr_len, unsigned short userport)
{
	...
	if (addr_type == IPV6_ADDR_MAPPED) {
		...
		err = dccp_v4_connect_TUv1(sk, (struct sockaddr *)&sin, sizeof(sin), userport);
		if (err) {
			icsk->icsk_ext_hdr_len = exthdrlen;
			icsk->icsk_af_ops = &dccp_ipv6_af_ops;
			sk->sk_backlog_rcv = dccp_v6_do_rcv;
...
static struct proto dccp_v6_prot = {
	...
	.connect_TUv1  = dccp_v6_connect_TUv1,
